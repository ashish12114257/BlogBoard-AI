...
def validator_node(state: BlogState) -> BlogState:
    ...
    # Tags for the generated blog: the domain plus any subtopics, deduplicated.
    subtopics = state.get("subtopics", "")
    extra_tags = [s.strip().lstrip("#") for s in subtopics.split(",") if s.strip()]
    tags = list(dict.fromkeys([d for d in [domain] if d] + extra_tags))

    # Draft mode used by the Spring Boot generation API: return the generated
    # data without persisting it to R2 / articles.json.
    if state.get("skip_storage"):
        print("  [AGENT] skip_storage enabled — returning draft without R2 persistence.")
        return {
            **state,
            "revision_needed": False,
            "title": title,
